        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-xl border border-gray-200 bg-white p-6">
              <label className="mb-3 block text-sm font-semibold text-gray-700">{t("settings.logo")}</label>
            ...
            {/* Company */}
            <div className="rounded-xl border border-gray-200 bg-white p-6">
              <h2 className="mb-4 text-sm font-semibold text-gray-700">{t("settings.companyName")}</h2>
            ...
            {/* Social */}
            <div className="rounded-xl border border-gray-200 bg-white p-6">
              <h2 className="mb-4 text-sm font-semibold text-gray-700">{t("settings.instagram")}</h2>
              <div className="space-y-4">
                <div>
          ...
          <div className="space-y-6">
            {/* Appearance */}
            <div className="rounded-xl border border-gray-200 bg-white p-6">
              <h2 className="mb-4 text-sm font-semibold text-gray-700">{t("settings.themeColor")}</h2>
              <div className="space-y-4">
                <div>
            ...
            {/* Loyalty */}
            <div className="rounded-xl border border-gray-200 bg-white p-6">
              <h2 className="mb-4 text-sm font-semibold text-gray-700">{t("settings.loyalty")}</h2>
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <Input type="number" min={1} max={50} value={loyaltyPointsRate} onChange={(e) => setLoyaltyPointsRate(e.target.value)} className="w-24" />
                      <span className="text-xs text-gray-500">{t("settings.loyaltyRateHint")}</span>
                    </div>
                  </div>
                  <div>
                    <label className="mb-1 block text-sm font-medium text-gray-700">{t("settings.loyaltyMinRedeem")}</label>
                    <div className="flex items-center gap-2">
                      <Input type="number" min={0} value={loyaltyMinRedeem} onChange={(e) => setLoyaltyMinRedeem(e.target.value)} className="w-24" />
                      <span className="text-xs text-gray-500">{t("settings.loyaltyMinRedeemHint")}</span>
                    </div>
                  </div>
                </div>
              )}
            ...
            {/* Payment methods */}
            <div className="rounded-xl border border-gray-200 bg-white p-6">
              <h2 className="mb-1 text-sm font-semibold text-gray-700">{t("settings.paymentMethods")}</h2>
              <p className="mb-3 text-xs text-gray-500">{t("settings.paymentMethodsHint")}</p>
                ...
                {paymentMethods.map((method, i) => (
                  <div key={i} className="flex items-center gap-2">
                    <Input
                      ...
                      className="flex-1"
                    />
                    <Button type="button" variant="danger" onClick={() => setPaymentMethods((prev) => prev.filter((_, j) => j !== i))}>×</Button>
                  </div>
                ))}
                <div className="flex items-center gap-2">
                  <Input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter") {
                        ...
                        if (v && paymentMethods.length < 10) { setPaymentMethods((prev) => [...prev, v]); setNewPaymentMethod(""); }
                      }
                    }}
                  />
            ...
            <div className="flex items-center gap-3">
              <Button type="submit" disabled={settingsMutation.isPending}>{t("common.save")}</Button>
